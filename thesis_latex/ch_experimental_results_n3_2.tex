...
the Scenario~A full-fusion energy in absolute terms: for FSRCNN
the partial-sized architecture consumes $1.01 \times 10^{4}$
versus $1.30 \times 10^{4}$ for full fusion, because the smaller
...
For the activation-dominant workloads on DepFiN, Scenario~B
even improves on the absolute energy of Scenario~A: the
partial-fusion cost on the smaller architecture is lower than the
full-fusion cost on the larger architecture, because the reduced
memories yield a lower per-access energy overhead without
sacrificing much of the DRAM elimination benefit.

The EDP verdict is identical in scope: three of eight
architecture--workload combinations favour fusion in both
scenarios (FSRCNN and MC-CNN on DepFiN, FSRCNN on Eyeriss).
The weight-dominant workloads continue to favour single-layer
...
fusion still outweighs the DRAM savings at 160\,pJ\,/\,byte,
