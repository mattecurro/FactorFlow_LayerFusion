...
12\,nm technology node).
For the off-chip DRAM energy we adopt the value of
160\,pJ\,/\,byte used by DepFiN~\cite{depfin}, which is itself
derived from JEDEC LPDDR4 specifications.
This value accounts for the full read/write access cost including I/O
and is considerably higher than the CACTI model estimate; it is also
consistent with the widely cited range of 100--200\,pJ\,/\,byte in
recent DNN accelerator literature~\cite{eyeriss_jssc}.
...
of $\sim\!225\times$ over FMEM in DepFiN and $\sim\!655\times$ over
...
($1.25\times$ the baseline) confirms this scaling:
on FSRCNN, DepFiN full fusion saves an even larger fraction of energy
versus the unfused baseline, because the unfused configuration must
pay the higher DRAM cost on $\sim$90\,M bytes of intermediate
traffic that fusion eliminates entirely.
