...
\subsection{DeFiNES}
DeFiNES \cite{mei2023defines} introduces a comprehensive framework for exploring the depth-first (DF) scheduling space. It builds upon LOMA \cite{Symons2021LOMA}, a analytical mapping framework designed to evaluate loop-order-based memory allocation, by extending its capabilities to cross-layer schedules.
...
[I will add a part with the differences with our approach: DeFiNES acts as an analytical wrapper around the standard single-layer ZigZag optimizer, cannot specify different dataflow for each layer.]
...
When discussing automated fusion exploration, it is critical to distinguish between macro-level graph decisions and micro-level dataflow mapping. \emph{Optimus}~\cite{optimus} exemplifies a state-of-the-art approach to the former. Operating as a graph-level compiler, Optimus employs memory-aware heuristics to mathematically determine exactly \emph{which} layers within a neural network should be grouped into fusion segments to avoid memory bottlenecks. 
