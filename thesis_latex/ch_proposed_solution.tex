...
\subsubsection{Dimension Proliferation with Aliasing and Decoupling}
\label{sec:ps-dim-proliferation}

Aliasing does not merely rename existing axes: it \emph{creates new
...
\begin{itemize}
  \item Three \textbf{aliased dimensions} ($Y_i$, $X_i$, $Z_i$) that
        ...
        has a height, a width, and a depth regardless of how it is
        indexed.
  ...
  \item Three \textbf{decoupled dimensions} ($C_i$, $R_i$, $S_i$) that
        belong exclusively to the new layer's weight coupling.
        These dimensions are genuinely new: they did not exist in the
        single-layer formulation and they introduce additional degrees
        of freedom for weight tiling.
\end{itemize}
%
...
Because the map-space cardinality
(Equation~\ref{eq:ms-cardinality}) depends \emph{multiplicatively}
on both the number of permutations and the number of factor
allocations, even the three aliased dimensions --- which add no new
physical data --- have a compounding effect on the search space.
For the 11-layer validation workload this proliferation inflates the
map space from roughly $10^{24}$ (single layer) to $10^{463}$ (fused),
as quantified in Section~\ref{sec:fds-mapspace}.

In summary, aliasing and decoupling together ensure that every
intermediate axis is an \emph{explicit} degree of freedom for the
mapper: it can be tiled independently, placed at any position in the
dataflow, and thereby influence stationarity and data reuse for its
layer --- at the cost of a linearly growing dimension set that must
be explored.
...
Each intermediate connection~$i$ introduces three alias pairs
...
single-triplet validator three times per layer connection --- once per
axis --- and short-circuiting to \emph{infeasible} as soon as any
axis fails.
For an $F$-layer fusion this yields $3(F\!-\!1)$ independent checks
rather than a single monolithic constraint over the entire
intermediate tensor.
...
stationary --- this is an \emph{output-stationary} dataflow for
layer~$0$.
Layer~$1$ sees $C_1$ as its innermost relevant loop: the output
$\text{Out}[\,z_1, p, q\,]$ does not depend on $C_1$, so the output
is stationary --- this is an \emph{output-stationary} dataflow for
layer~$1$ as well.
...
This property follows directly from the modelling choices described
