...
along dimension~$d$ that the level must store simultaneously.
...
In practice, the objective is not merely to fit tiles, but to fill
each memory as fully as possible, thereby maximizing the amount of
data that is reused before being evicted.
...
Spatial fanout levels replicate all subsequent levels into a mesh of
concurrent instances.
A mapping must specify which loop dimension(s) are \emph{spatially
unrolled} across each fanout level and how many iterations each
instance executes.
This is modeled identically to tiling: the prime-factor copies of a
...
When the unrolled dimension is \emph{affine} for an operand---that
is, it appears in a sum of indices like $p + r$---adjacent instances
...
\begin{enumerate}
  ...
  \item \textbf{Halo reuse (temporal, affine).}
    ...
    operand---that is, it participates in a sum of indices such as
    $p + r$ (Definition~\ref{def:affine-dim})---consecutive tiles
    \emph{overlap} due to the sliding-window access pattern.
    Only the non-overlapping slice, called the \emph{halo}, needs to
    be fetched from the outer memory; the remainder is reused from the
    previous iteration.
