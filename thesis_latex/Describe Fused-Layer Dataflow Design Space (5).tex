...
Chapter~\ref{ch:background} has established the mapping problem for a
...
Chapter~\ref{ch:background} also has introduced layer fusion as a strategy
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\textwidth]{Images/stride_1.png}
  \caption{Example of output tile size 2x2 with stride 1}
  \label{fig:fds-tile-deps-no-stride}
\end{figure}


\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\textwidth]{Images/stride_2.png}
  \caption{Example of output tile size 2x2 with stride 2}
  \label{fig:fds-tile-deps-no-stride_2}
\end{figure}


As discussed in Section~\ref{sec:fused-tiling}, selecting an output spatial tile of size $P \times Q$ for the final layer mathematically forces every preceding layer to compute a progressively larger tile to account for overlapping filter halo. Accounting for independent strides along the spatial dimensions, this constraint propagates backward according to:
\begin{equation*}
\label{eq:cascading-tiles-recap}
\text{Layer } i \text{ (intermediate footprint)}: \quad
  \big(\text{stride}_h \cdot (P - 1) + R\big)
  \;\times\;
  \big(\text{stride}_w \cdot (Q - 1) + S\big)
\end{equation*}
...
Two principal models exist in the literature:
...
share horizontal halo.
The optimal choice depends on the filter shapes, strides and the relative
costs of fetching horizontal vs.\ vertical non-overlapped slices.
...
Within each fused layer, the single-layer mapping (intra-layer mapping) degrees of freedom
apply unchanged:
\begin{itemize}
...
Section~\ref{sec:data-reuse} has introduced the fundamental reuse
mechanisms available in single-layer mapping: stationarity (temporal
reuse), halo overlap, and spatial multicast.
Section~\ref{sec:inter-intra-reuse} then has noted that fusion introduces
...
Consider a 1-Dimensional example with a $3\times 3$ filter ($R = 3$, stride 1).
...
A common scenario is 2-Dimensional tiling with a row-major iteration order.
...
The key insight that reduce this immense map-space is that a vast
...
\textbf{Constraint-based pruning} eliminates part of these configurations
...
\begin{enumerate}
  \item \textbf{Dataflow constraints} fix the loop ordering at levels
        ...
        constrained to the Weight Memory (WMEM) level.
...
The critical point of constraint-based pruning is neither an
approximation nor a heuristic that might discard high-quality solutions: the
