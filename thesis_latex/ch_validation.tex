...
\begin{enumerate}
  \item \textbf{DRAM bandwidth partitioning.}
        The paper reports an aggregate external I/O bandwidth of
        12\,B/cycle (96\,bits) but does not specify how this bandwidth
        is split between reads and writes.
        In the model, we allocate 2\,B/cycle for reads and 4\,B/cycle
        for writes.
        This reflects the observation that, in fused mode, DRAM reads
        (input activations + weights) occur in a streaming fashion
        before writes (output activations), and we verified that the FMEM
        bandwidth (132\,B/cycle read, 128\,B/cycle write) is never the
        bottleneck under this assumption.

  \item \textbf{Absence of precise DRAM bandwidth data.}
        The DepFiN paper does not provide a cycle-accurate DRAM
        bandwidth model; the reported latency values include unspecified
        stall effects from external memory.
        This prevents us from precisely matching the absolute latency
        in cycles but does not affect the ability to validate the
        \emph{ideal} (compute-bound) latency, which depends only on
        the number of MAC operations and the PE utilization.
...
\subsection{Memory Stalls}
\label{sec:val-stalls}

Due to the absence of precise DRAM bandwidth specifications in the
DepFiN paper, a direct comparison of total latency with stalls is
not possible.
However, the stall analysis from the model provides useful insights:
\begin{itemize}
  \item \textbf{DRAM} introduces 3,766,468 stall cycles, increasing
        the total latency from the ideal 37.94\,M to 45.35\,M cycles
        ($+19.5\,\%$).
        This stall is caused by the limited DRAM bandwidth of
        12\,B/cycle (2\,B/cycle read + 4\,B/cycle write in the model
        allocation), which cannot sustain the data delivery rate
        required by 2,048 PEs running at peak throughput.
  \item \textbf{FMEM} introduces only 483 stall cycles—effectively
        zero.
        The 132\,B/cycle read bandwidth and 128\,B/cycle write
        bandwidth of FMEM are more than sufficient to feed the PE
        array without any bottleneck.
        This confirms that the \emph{on-chip} memory hierarchy of
        DepFiN is well-dimensioned for the target workload.
  \item \textbf{WMEM} introduces zero stall cycles, meaning the
        weight delivery from the Weight Memory is never the bottleneck.
\end{itemize}

The key validation evidence from the stall analysis is that
\textbf{FMEM does not introduce stalls}.
This means the Feature Memory bandwidth is sufficient to support
the depth-first tiled execution without ever becoming a bottleneck,
which is consistent with DepFiN's architectural claim that the large
on-chip FMEM enables seamless multi-layer fusion at full throughput.
...
\begin{enumerate}
  ...
  \item The \textbf{FMEM capacity} is verified to be sufficient for
        the entire fused pipeline, with no intermediate data spills—confirming that the 1056\,KB Feature Memory provides
        fully-cached intermediate storage.
  \item The \textbf{FMEM bandwidth} introduces negligible stall
        (483 cycles out of 45.35\,M), validating that the on-chip
        data delivery is well-matched to the PE throughput.
\end{enumerate}
