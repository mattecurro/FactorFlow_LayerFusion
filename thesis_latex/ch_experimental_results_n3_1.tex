...
and at 160\,pJ\,/\,byte the DRAM energy eliminated by fusion
vastly exceeds the extra on-chip cost of the somewhat enlarged
feature memory (576\,KB for FSRCNN, 522\,KB for MC-CNN).
...
elevated per-access energy that compounds across billions of MACs.
...
entries, each contributing to per-access energy.
Despite the high DRAM cost, the sheer volume of on-chip accesses
...
Full fusion delivers latency benefits whose magnitude depends on
...
For the weight-dominant workloads the behaviour diverges across
architectures.
...
$30.3\%$.
The cause is a \emph{suboptimal joint tiling structure} at the
DRAM level.
In single-layer mode each layer independently optimises its
loop tiling: the DRAM outer loop iterates only over that layer's
own spatial dimensions, and every memory level is perfectly
balanced (compute-bound, no stalls).
In full fusion, all 17~layers share the DRAM level, and the
mapper must place the spatial-height factors ($Y_i$) of
\emph{every} layer at this outermost level because the
on-chip memories are already saturated by the 10--14\,MB of
weights.
This inflates the DRAM iteration count for the early, small
layers far beyond what they need in isolation.
A per-layer breakdown for ResNet18 confirms the effect: the
first five convolutional layers (L0--L4) suffer a
$2$--$7\times$ DRAM-latency inflation, contributing $+1.6$\,M
excess clock cycles; the three stride-2 transition layers
(L5, L9, L13) add another $+1.2$\,M due to the doubling of
DRAM iterations at each spatial halving.
Meanwhile, the second layer of every residual pair (L6--L8,
L10--L12, L14--L16) shows virtually no overhead because its
single-layer latency is already dominated by the same DRAM
bottleneck.
Eyeriss avoids this penalty because its distributed per-PE
register architecture keeps each layer's weight factors local,
allowing the mapper to balance DRAM and compute without a
shared memory contention point.
...
is still slightly more efficient on EDP.
...
(Eyeriss) favour partial or no fusion.
