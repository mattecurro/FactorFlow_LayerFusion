...
Chapter~\ref{ch:fused-design-space} has characterized the fused-layer
...
This chapter presents how we have \textbf{extended FactorFlow} to model,
...
\begin{enumerate}
  \item \textbf{No per-layer mapping freedom.}
        ...
        nested dim-sum, the fused layers cannot have independent stationarities
        (Section~\ref{sec:data-reuse}).

  \item \textbf{Forced back-to-back multiply accumulation (MAC) execution.}
...
intermediate activation, and to register them in the coupling  (Section \ref{sec:bg-coupling-shape}) so
that:
%
\begin{itemize}
...
different layers become disjoint, leading to a property we
...
$\mathcal{D}$ can be partitioned into
...
\begin{itemize}
  \item Three \textbf{aliased dimensions} ($Y_i$, $X_i$, $Z_i$) that
        ...
        and cannot appear as standalone loops (see the nested
...
\paragraph{An example: Fused 10-layer configuration.}
...
\begin{table}[htbp]
  \centering
  \caption{Map-space size comparison: single layer vs.\ 10-layer
...
multi-memory level loop nest, the producer dimension (i.e.: $X$, $Y$, $Z$) may be tiled
across several hierarchy levels, while the consumer dimensions (i.e.: $Q$ and $S$, $P$ and $R$, $C$) that fall \emph{between} two consecutive producer loop instances
can also belong to different levels.
For instance, considering the cross-level validation for the producer dimension width $X$.
...
\begin{equation}
\label{eq:ps-pruning-check-width}
...
The constraint is analogous for each physical axis, with a simplification for the Z axis (because there is not an affine dimension in that case).
\begin{equation}
\label{eq:ps-pruning-check-height}
  P_{\text{zone}} + R_{\text{zone}} - 1 \;\le\; Y_{\text{cum}}.
\end{equation}
\begin{equation}
\label{eq:ps-pruning-check-channel}
  C_{i+1, \text{zone}} \;\le\; Z_{i, \text{cum}}.
\end{equation}

Since the three axes index orthogonal dimensions of the same tensor, 
the pruning constraint can be evaluated \emph{independently} for each
triplet: satisfying the width constraint
(Equation~\ref{eq:ps-pruning-check-width}) neither implies nor precludes
satisfaction of the height constraint (Equation~\ref{eq:ps-pruning-check-height}), and likewise for the channel
constraint (Equation~\ref{eq:ps-pruning-check-channel}).
...
\subsubsection{Two layer fusion example}
\label{sec:ps-pruning-example}

To make the zone-based pruning concrete, this section walks through
the width axis ($Q$, $S$, $X$) of a two-layer fusion (height and channel axes are
validated independently with the same procedure).
Throughout this example, subscripts on dimension names denote the
\emph{hierarchy level} (not the layer index): $Q_1$ is the $Q$ loop
at the DRAM level, $X_3$ is the $X$ loop at the Register level,
and so on.

\paragraph{Setup.}
Consider a width-axis configuration with full extents
$X = 18$, $Q = 16$, $S = 3$ (satisfying the global constraint
$Q + S - 1 = 18 \le 18 = X$, with stride width = 1).
The architecture has three memory levels: DRAM (level~1), Global
Buffer (level~2), and Register (level~3).
A candidate mapping tiles these dimensions across the three levels as
follows (outermost to innermost):

\begin{center}
\small
\begin{tabular}{c l c l}
  \toprule
  \textbf{Loop} & \textbf{Iterations} & \textbf{Level} \\
  \midrule
    $Q_1$ & 4 & DRAM \\
    $X_1$ & 3 & DRAM \\
    $S_1$ & 1 & Global Buffer \\
...
\noindent
The three $X$ loops ($X_1, X_2, X_3$) define the zone boundaries.
Between each pair of consecutive $X$ loops, the validator collects the
$Q$ and $S$ iterations that fall in that region, regardless of which
memory level they belong to.

\paragraph{Initial condition.}
The innermost loop with more than one iteration is $X_3$
(3~iterations), which is a producer dimension.
The initial condition is satisfied: the first non-trivial iterations
of the nest produce intermediate data before any consumer loop
attempts to read.

\paragraph{Zone-by-zone validation.}
The three $X$ loops create three zones, validated from innermost
outward.
Table~\ref{tab:pruning-zones} summarizes the check.

\begin{table}[htbp]
  \centering
  \caption{Zone-by-zone width-axis pruning validation for the
           example mapping.  Every zone satisfies
           $Q_{\text{zone}} + S_{\text{zone}} - 1 \le X_{\text{cum}}$.}
  \label{tab:pruning-zones}
  \small
  \begin{tabular}{l c c c c c}
    ...
    1\,(X$_3 \!\to\! $X$_2$)
      ...
      & $1 \!\times\! 3 = 3$
      & $3$
      & $1\!+\!3\!-\!1 = 3 \le 3\;\checkmark$ \\[3pt]
    2\,(X$_2 \!\to\! $X$_1$)
      & $S_1,\, Q_2$
      & $4$
      & $1$
      & $2 \!\times\! 3 = 6$
      & $4\!+\!3\!-\!1 = 6 \le 6\;\checkmark$ \\[3pt]
    3\,(global)
      & $Q_1$
      & $4 \!\times\! 4 \!\times\! 1 = 16$
      & $1 \!\times\! 1 \!\times\! 3 = 3$
      & $3 \!\times\! 2 \!\times\! 3 = 18$
      & $16\!+\!3\!-\!1 = 18 \le 18\;\checkmark$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
All three zones pass: the mapping is feasible on the width axis.
 ...
 \begin{figure}[htbp]
   ...
   \label{fig:pruning-zones}
 \end{figure}

\paragraph{A failing counterexample.}
To illustrate what the pruning catches, consider the \emph{same} loop
ordering but with a different tiling: move one factor of~$X$ from the
Global Buffer level to the DRAM level, setting $X_1 = 6$ and
$X_2 = 1$. Zone~2 now fails:
\[
  Q_{\text{zone}} + S_{\text{zone}} - 1 = 4 + 3 - 1 = 6 > 3 = X_{\text{cum}}.
\]
The pruning validator rejects this mapping immediately, preventing
the cost model from evaluating an execution schedule that would read
...
Equations~\ref{eq:ps-pruning-check-height}, \ref{eq:ps-pruning-check-channel},
...
\textbf{Coupling} \ref{sec:bg-coupling-shape}.
...
\begin{itemize}
  \item A \texttt{dataflow\_constraints} list, fixing the loop
        ordering at that level.  
  \item A \texttt{factors\_constraints} dictionary, fixing the number
        of iterations of specified dimensions at that level.
\end{itemize}
...
Stationarity (Section \ref{sec:data-reuse} is determined by which dimensions iterate at
...
For instance, if we consider a two-layer fusion where the dataflow at a
...
If we consider a different ordering, e.g.\
...
point for the map-space exploration engine.
...
This section provides an overview of our codebase, focusing
...
\begin{enumerate}
  ...
  \item \textbf{Feasibility pruning} 
        ...
        (Section~\ref{sec:ps-pruning}) is evaluated on the new
        resulting mapping.
...
(\textit{breadthfirst\_local, linear, quadratic,
exponential, hybrid}) differ in how they select
